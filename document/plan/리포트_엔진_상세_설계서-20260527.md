...
| 기준 템플릿 | `document/report/우리은행_월간_리포트.html` | A4 portrait HTML | Stage 3 HTML 바인딩 기준 |
| 국민은행 템플릿 이미지 | `document/report/국민은행_월간_리포트.png` | preview image | `tpl_kb_monthly_guidebook`의 Stage 3 멀티모달 시각 기준 |
| 우리은행 템플릿 이미지 | `document/report/우리은행_월간_리포트.png` | preview image | `tpl_woori_monthly_report`의 Stage 3 멀티모달 시각 기준 |
...
| `LLM_STAGE_BATCH_SIZE` | `12` | Stage 2 batch 처리 기준 |
...
| 항목 | 내용 |
| --- | --- |
| 목적 | 리포트의 시각 기준이 되는 HTML 템플릿과 preview image를 확정한다. |
| 입력 | `templateId`, `sourceHtml`, `previewImage`, page 설정 |
| 처리 | `report_templates.json`에서 선택한 `templateId`의 템플릿 메타를 조회한다. |
| 출력 | `TemplateContext` |
| 실패 처리 | 템플릿 HTML 또는 이미지 파일이 없으면 `TEMPLATE_NOT_FOUND`로 job 실패 처리 |
...
```json
{
  ...
  "previewImage": "document/report/국민은행_월간_리포트.png",
  "page": {
    "size": "A4",
...
| 처리 | Novita LLM 또는 sample adapter로 componentKey별 HTML fragment 생성 |
| 출력 | `Stage2Component[]` 5개 |
| 실패 처리 | 5개 componentKey가 모두 없으면 `COMPONENT_SOURCE_INCOMPLETE` |
...
| `novita` | 실제 LLM 검증 | `etf_stage2_component_sources.json` payload를 Novita LLM에 전달 |
...
| 출력 | `ReportHtmlDraft` |
| 실패 처리 | HTML skeleton 누락, component 누락, chart container 누락 시 `LAYOUT_GENERATION_INVALID` |
...
- CSS는 Stage 3에서만 생성한다.
- 외부 네트워크 리소스를 참조하지 않는다.

차트는 PoC 기본값으로 백엔드 SVG/이미지 렌더링을 사용한다. Stage 2의 `chartSpec`은 차트 생성 입력 계약으로 유지하고, Stage 3 HTML에는 생성된 inline SVG 또는 이미지 참조를 삽입한다.

### 4.4 Stage 4: 렌더링 및 버전 저장

| 항목 | 내용 |
| --- | --- |
| 목적 | HTML을 preview image로 렌더링하고 버전 단위로 저장한다. |
| 입력 | `ReportHtmlDraft` |
| 처리 | HTML 저장, PNG preview 생성, version metadata 저장 |
| 출력 | `ReportVersion` |
| 실패 처리 | 렌더링 실패 시 `RENDER_FAILED`, preview 누락 시 `PREVIEW_NOT_CREATED` |
...
| Stage 2 컴포넌트 생성 | 가능 | 중 | componentSources/sample components가 60개로 준비됨 | sample adapter 우선, Novita adapter는 componentKey별 prompt 호출 | `datasetId`, `monthId`, `componentSources[]`, `LLM_*` 설정 | `Stage2Component[]` 5개 | LLM이 style/class 삽입, 수치 변경 | HTML 금지 문자열 검사, componentKey 5개 확인 |
| Stage 3 레이아웃/CSS 생성 | 조건부 가능 | 상 | 멀티모달 레이아웃 품질이 LLM 응답 안정성에 의존 | 템플릿 preview image와 5개 컴포넌트를 Novita에 전달해 단일 HTML 생성 | `TemplateContext`, `Stage2Component[]` | `ReportHtmlDraft` | A4 overflow, 원천 데이터 변형, CSS 과도 생성 | 렌더링 후 이미지 검증, 원천 수치 비교 |
| Stage 4 렌더링/버전 저장 | 가능 | 중 | HTML->이미지 렌더링은 표준 브라우저 자동화로 구현 가능 | Playwright screenshot, version directory 저장 | HTML draft, jobId, version number | `ReportVersion` | 브라우저 폰트 차이, 렌더링 실패 | preview 파일 생성, viewport/A4 크기 확인 |
...
| `RENDER_FAILED` | Stage 4 | renderer fallback 시도 후 실패 |
...
| Stage 3 HTML 생성 | 단일 HTML 문서, 5개 섹션 포함 |
| Stage 4 렌더링 | version별 `report.html`, `preview.png` 생성 |
