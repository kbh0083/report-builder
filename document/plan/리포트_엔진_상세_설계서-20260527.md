...
| 처리 | Novita LLM 멀티모달 호출로 A4 layout과 CSS를 생성하고, local validator 및 correction loop로 Stage 2 보존 계약을 검증한다. |
| 출력 | document-local CSS를 포함한 `ReportHtmlDraft` |
| 실패 처리 | HTML skeleton 누락, component 누락, CSS 누락, chart placeholder 누락, Stage 3 chart 렌더링 시도, source fragment 복원 실패 시 `LAYOUT_GENERATION_INVALID` |

Stage 3 LLM prompt는 아래 제약을 반드시 포함한다.

- 결과는 단일 HTML 문서로 반환한다.
- A4 portrait 크기를 유지한다.
- 본문은 Stage 2 component key인 `performance_chart`, `performance_summary`, `top_holdings`, `review`, `outlook`를 모두 포함한다.
...
Stage 3 검수/보정 흐름은 아래로 고정한다.

1. initial HTML을 생성한다.
2. `validate_report_draft()`가 complete HTML, document-local CSS, component id, chart placeholder, source fragment, footer normal flow, chart rendering 금지를 검증한다.
3. 실패하면 correction prompt를 최대 3회 호출한다.
4. correction 3회 후에도 non-chart source fragment가 누락/변형되면 local repair가 해당 component 내부 HTML을 Stage 2 원문으로 복원하고 validator를 다시 실행한다.
5. 모든 attempt는 `backend/log/{jobId}/03_layout/layout_validation_loop.json`과 `03_layout/attempts/attempt_*_report_draft.html`에 기록한다.
...
| 입력 | `ReportHtmlDraft`, `performance_chart.chartSpec`, `TemplateContext.chartProfile`, `jobId`, `version number` |
| 처리 | template chart profile 기준 chart type 결정, SVG/이미지 생성, `data-chart-placeholder` 교체, HTML 저장, PNG preview 생성, version metadata 저장 |
| 출력 | `ReportVersion` |
| 실패 처리 | chartSpec 누락, chart placeholder 누락/교체 실패, HTML 렌더링 실패, preview 누락 시 `RENDER_FAILED` |

Stage 4 처리 순서는 아래로 고정한다.

1. Stage 2의 `performance_chart.chartSpec`과 template `chartProfile.performance_chart`를 읽는다.
2. template image에 chart가 있으면 `detectedChartType`, 없으면 `fallbackChartType`을 effective chart type으로 사용한다.
3. `ChartRenderer`가 effective chart type으로 백엔드 SVG 또는 image asset을 렌더링한다.
4. Stage 3 HTML의 `data-chart-placeholder` 컨테이너를 생성된 chart markup 또는 image reference로 교체한다.
5. 교체된 HTML을 `runs/{jobId}/v{version}/report.html`에 저장한다.
6. `HTML Renderer`가 저장된 HTML을 열어 `preview.png`를 생성한다.
7. `VersionStore`가 `ReportVersion(status="verifying")` metadata를 저장한다.
8. `render_result.json`에는 `originalChartSpecType`, `effectiveChartType`, `chartTypeSource`를 기록한다.
...
| 항목 | 내용 |
| --- | --- |
| 목적 | template image와 preview image를 비교해 레이아웃/스타일 품질을 검증하고 수정 지시를 생성한다. |
| 입력 | template preview image, `ReportVersion.previewImagePath`, 현재 HTML, Stage 2 component 보존 계약 |
| 처리 | Novita LLM 멀티모달 검증, issue 생성, 수정 지시 반영 |
| 출력 | `VerificationResult` 또는 다음 HTML draft |
| 실패 처리 | 반복 한도 초과 시 loop에 `maxIterationsExceeded=true`를 기록하고 Stage 6 best failed fallback 대상이 된다. |
...
| `template_mismatch` | 템플릿 대비 layout/style/brand 구조가 다름 | header logo, footer, section marker, table style 불일치 |

Stage 5의 데이터/템플릿 역할은 분리한다. 템플릿 이미지는 page geometry, header/footer 구조, section marker, chart/table/list style, spacing, palette의 기준이다. 템플릿 이미지 안의 샘플 텍스트, 수치, 종목명, 섹션명, 표 컬럼명은 현재 데이터와 일치해야 하는 기준이 아니다. 데이터 검증 기준은 Stage 2 component HTML과 현재 report HTML이다.
...
| 항목 | 내용 |
| --- | --- |
| 목적 | 검증을 통과한 version 또는 반복 한도 소진 시 best failed version을 최종 산출물 위치로 확정한다. |
| 입력 | `VerificationAttempt[]`, `maxIterationsExceeded` |
| 처리 | passed version 우선 final alias 생성, 필요 시 best failed fallback final 생성 |
| 출력 | 최종 HTML 경로와 preview image 경로 |
| 실패 처리 | 통과 version이 없고 best failed fallback 조건도 아니면 `NO_PASSED_VERSION` |

passed version이 있으면 해당 version을 `final/`로 복사하고 `verificationPassed=true`, `finalizationReason=passed_verification`를 기록한다. `maxIterationsExceeded=true`이고 passed version이 없으면 `bestFailedVersion`을 `final/`로 복사하되, 검수 통과물이 아니므로 `verificationPassed=false`, `finalizationReason=best_failed_after_max_iterations`를 기록한다.
...
| Stage 3 레이아웃/CSS 생성 | 조건부 가능 | 상 | 멀티모달 레이아웃 품질이 LLM 응답 안정성에 의존 | 템플릿 preview image와 5개 컴포넌트를 Novita에 전달해 document-local CSS 포함 단일 HTML 생성. chart는 placeholder만 유지하고 validator/correction/local repair를 수행 | `TemplateContext`, `Stage2Component[]` | CSS 포함 `ReportHtmlDraft` | A4 overflow, 원천 데이터 변형, CSS 누락/과도 생성, Stage 3 chart 렌더링 시도 | CSS 포함 검사, 원천 수치 비교, chart rendering markup 금지 검사, validation loop artifact 확인 |
| Stage 4 차트/HTML 렌더링 및 버전 저장 | 가능 | 중 | chartSpec은 Stage 2에서 구조화되어 있고 chart type은 template chart profile로 결정 가능 | ChartRenderer로 template chart profile 기준 SVG/이미지를 만들고 placeholder를 교체한 뒤 Playwright screenshot과 version directory 저장 | HTML draft, `performance_chart.chartSpec`, `chartProfile`, jobId, version number | chart 포함 `ReportVersion` | chartSpec 불완전, chartProfile 누락, placeholder 교체 실패, 브라우저 폰트 차이, 렌더링 실패 | chart metadata, placeholder 교체 검사, preview 파일 생성, viewport/A4 크기 확인 |
| Stage 5 이미지 검증/수정 루프 | 조건부 가능 | 상 | 이미지 검증은 LLM 판단 품질과 수정 지시 품질에 의존 | template image와 preview image를 함께 전달해 issue JSON을 받고 Stage 3 재생성 | template image, preview image, current HTML, maxIterations | `VerificationResult`, revised HTML | false pass, 반복 수렴 실패 | issue schema 검사, 반복 한도, best failed fallback, 수동 spot check |
...
| `LAYOUT_GENERATION_INVALID` | Stage 3 | correction prompt 최대 3회 후 local repair를 시도하고, 그래도 validator 실패 시 job 실패 |
| `RENDER_FAILED` | Stage 4 | chart renderer 또는 HTML renderer fallback 시도 후 실패 |
| `VERIFICATION_MAX_ITERATIONS_EXCEEDED` | Stage 5 | loop에 `maxIterationsExceeded=true`와 best failed 후보를 기록 |
| `NO_PASSED_VERSION` | Stage 6 | passed version이 없고 best failed fallback 조건도 아니면 job 실패 |
