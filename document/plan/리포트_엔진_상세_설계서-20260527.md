...
| 출력 | `ReportHtmlDraft` |
| 실패 처리 | HTML skeleton 누락, component 누락, chart placeholder 누락, Stage 3 chart 렌더링 시도 시 `LAYOUT_GENERATION_INVALID` |
...
| Stage 3 레이아웃/CSS 생성 | 조건부 가능 | 상 | 멀티모달 레이아웃 품질이 LLM 응답 안정성에 의존 | 템플릿 preview image와 5개 컴포넌트를 Novita에 전달해 단일 HTML 생성. chart는 placeholder만 유지 | `TemplateContext`, `Stage2Component[]` | `ReportHtmlDraft` | A4 overflow, 원천 데이터 변형, CSS 과도 생성, Stage 3 chart 렌더링 시도 | 렌더링 후 이미지 검증, 원천 수치 비교, chart rendering markup 금지 검사 |
