...
| Chart Renderer | template chart profile 기준으로 chart type을 결정하고 Stage 3 chart placeholder 교체에 필요한 payload를 만든다. | `chartSpec`, `chartProfile`, `data-chart-placeholder`, theme | inline SVG 또는 image path |
| Stage 3 Layout Service | 템플릿 이미지와 컴포넌트를 layout/CSS 포함 통합 HTML로 조립하고 local validator/correction/local repair로 보존 계약을 검증한다. | preview image, components | chart placeholder 포함 HTML draft + CSS |
...
| Stage 5 Verification Service | template image와 preview image 비교 검증, 데이터 보존 검증, 수정 지시 생성 | template image, preview image, HTML | `VerificationResult` |
...
`Stage 5 Verification Service`는 template image, preview image, 현재 HTML을 함께 검증하고, 발견된 문제를 표준 issue type으로 정규화한다. 템플릿 이미지는 layout/style 기준이고 데이터 기준은 Stage 2 component HTML과 현재 report HTML이다.
...
| `template_mismatch` | 템플릿 대비 layout/style/brand 구조 불일치 | header/footer/section marker/table/chart style 수정 지시 |
...
| `final/*` | 통과 version 또는 best failed fallback alias |

감사/디버깅 artifact는 `backend/log/{jobId}/` 하위에 stage별로 저장한다. 주요 파일은 `03_layout/layout_validation_loop.json`, `03_layout/attempts/attempt_*_report_draft.html`, `04_render/render_result.json`, `05_verification/verification.json`, `05_verification/verification_loop.json`, `06_final/final_result.json`이다.
...
| Stage 3 레이아웃/CSS 생성 | 조건부 가능 | 상 | 템플릿 이미지 기반 품질 판단이 LLM에 의존 | 멀티모달 prompt로 document-local CSS 포함 단일 HTML 생성 후 validator, correction 최대 3회, source fragment local repair를 수행한다 | template image, components | HTML draft + CSS | CSS 누락, overflow, clipping, 데이터 변경, Stage 3 chart 렌더링 시도, footer collision | CSS 포함 검사, 데이터 대조, chart rendering markup 금지 검사, footer absolute/fixed 금지 검사, validation loop artifact 확인 |
| Stage 4 차트/HTML 렌더링 및 버전 저장 | 가능 | 중 | chartSpec은 Stage 2에서 구조화되어 있고 chart type은 template chart profile로 결정 가능 | ChartRenderer가 template chart profile 기준 chart SVG/이미지를 만들고 `data-chart-placeholder`를 교체한 뒤 Playwright screenshot 후 version directory 저장 | HTML draft, chartSpec, chartProfile, job metadata | chart 포함 `ReportVersion` | chartSpec 불완전, chartProfile 누락, placeholder 교체 실패, 폰트/브라우저 차이 | chart metadata, placeholder 교체, preview 생성, A4 pixel 비율 확인 |
| Stage 5 이미지 검증/수정 루프 | 조건부 가능 | 상 | LLM 시각 검증 품질과 반복 수렴성이 변수 | template image와 preview image를 비교하고 HTML/Stage 2 기준 데이터 보존을 검증한 뒤 issue JSON 생성 후 재생성 | template image, preview image, HTML, iteration | `VerificationResult` | false positive/pass, 반복 미수렴 | max iteration, issue schema, best failed fallback, 수동 샘플 검수 |
...
| 검증 루프 미수렴 | Orchestrator | max iteration 도달 후 best failed version을 final로 복사하고 `verificationPassed=false`로 기록 |
