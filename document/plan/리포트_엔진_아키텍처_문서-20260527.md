...
| Stage 3 레이아웃/CSS 생성 | 조건부 가능 | 상 | 템플릿 이미지 기반 품질 판단이 LLM에 의존 | 멀티모달 prompt로 단일 HTML 생성, chart는 backend SVG/이미지 삽입 | template image, components, chart asset | HTML draft | overflow, clipping, 데이터 변경 | render preview, 데이터 대조, visual 검증 |
