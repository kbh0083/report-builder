...
flowchart LR
    ...
    Orchestrator --> HtmlRenderer["HTML Renderer"]
...
| Chart Renderer | `chartSpec`을 SVG/이미지로 변환 | `chartSpec`, theme | inline SVG 또는 image path |
| Stage 3 Layout Service | 템플릿 이미지와 컴포넌트를 통합 HTML로 조립 | preview image, components | HTML draft |
| HTML Renderer | HTML을 preview image로 렌더링 | HTML draft | `preview.png` |
| Version Store | HTML, preview, 검증 결과를 version 단위로 저장 | version payload | `ReportVersion` |
...
Stage 3의 시각 기준 이미지는 선택한 `templateId`의 `report_templates.json` 항목을 사용한다. `tpl_kb_monthly_guidebook`은 `document/report/국민은행_월간_리포트.png`, `tpl_woori_monthly_report`는 `document/report/우리은행_월간_리포트.png`를 참조한다. 현재 국민은행 전용 HTML 템플릿 파일은 없으므로 두 템플릿 모두 `sourceHtml`은 기존 `document/report/우리은행_월간_리포트.html`을 유지한다. 데이터셋과 템플릿은 독립 선택 단위이므로 repository는 판매사 일치 여부를 검사하지 않는다.
...
| --- | --- | --- |
| `performance_chart` | `chart` | `chartSpec`을 `Chart Renderer`에 전달해 백엔드 SVG/이미지를 만든다. |
...
| `version.created` | Stage 4 저장 완료 | `version`, `previewImagePath` |
...
| Stage 3 레이아웃/CSS 생성 | 조건부 가능 | 상 | 템플릿 이미지 기반 품질 판단이 LLM에 의존 | 멀티모달 prompt로 단일 HTML 생성, chart는 `data-chart-placeholder`만 유지하고 footer는 normal flow에 둔다 | template image, components | HTML draft | overflow, clipping, 데이터 변경, Stage 3 chart 렌더링 시도, footer collision | render preview, 데이터 대조, chart rendering markup 금지 검사, footer absolute/fixed 금지 검사 |
| Stage 4 렌더링/버전 저장 | 가능 | 중 | HTML preview 렌더링은 브라우저 자동화로 구현 가능 | Playwright screenshot 후 version directory 저장 | HTML draft, job metadata | `ReportVersion` | 폰트/브라우저 차이 | preview 생성 확인, A4 pixel 비율 확인 |
...
| 렌더링 실패 | HTML Renderer | fallback renderer 시도 후 실패 |
